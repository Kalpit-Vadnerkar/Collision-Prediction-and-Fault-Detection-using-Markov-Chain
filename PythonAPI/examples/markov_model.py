...
def game_loop(args):
    ...
    try:
        ...
        if args.agent == "Basic":
            agent = BasicAgent(world.player, 90)
        ...
        start_time = sim_world.get_snapshot().timestamp.elapsed_seconds
        interval = 0.3
        X1 = round(world.player.get_location().x)
        Y1 = round(world.player.get_location().y)
        ...
        while True:
            ...
            current_time = sim_world.get_snapshot().timestamp.elapsed_seconds
            if current_time - start_time >= interval: # 1/2 second has passed in the simulation
                start_time = sim_world.get_snapshot().timestamp.elapsed_seconds
                X2 = round(world.player.get_location().x)
                Y2 = round(world.player.get_location().y)

                Vx2 = round(world.player.get_velocity().x)
                Vy2 = round(world.player.get_velocity().y)

                dx = X2 - X1
                dy = Y2 - Y1
                theta1 = math.degrees(math.atan2(dy-Vy1, dx-Vx1))
                theta2 = math.degrees(math.atan2(dy-Vy2, dx-Vx2))
                Dtheta = round((theta2 - theta1)/10)
            ...
            if agent.done():
                ...
                if args.loop:
                    ...
                    agent.set_target_speed(random.choice(speeds))
...
states = []
for i in range(-5, 6): # Vx (Velocity)
    for j in range(-5, 6): # Vy (Velocity)
        for k in range(-9, 10): # Theta (Change in orientation)
            state = (i, j, k)
            states.append(state)
states.append("Enter")
states.append("Exit")
